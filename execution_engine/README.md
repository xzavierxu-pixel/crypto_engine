...
The execution engine loads the deploy artifact under:

```text
execution_engine/deploy/baseline
```

As of 2026-05-20 this directory is generated by the second, full-data training step:

```powershell
python scripts/model/train_online_full_train.py `
...
Offline acceptance still comes from the first split training run. The deploy artifact records that score under `offline_validation_metrics`; the full-train metrics are in-sample diagnostics only.

Current deploy manifest summary:
...
The execution config should leave `baseline.model_file` and `baseline.calibrator_file` as `null` so the loader resolves filenames from `artifact_manifest.json`.
