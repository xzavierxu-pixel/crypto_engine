...
DEFAULT_CONFIG_PATH = Path("experiments/configs/20260611_catboost_calendar_fm_side_coordinate_search.yaml")
LEAKAGE_BLOCKLIST = {
    "target",
    "future_close",
    "abs_return",
...
def _coordinate_search(
    ...
    p_up: pd.Series,
    config: dict[str, Any],
) -> tuple[pd.DataFrame, dict[str, Any], dict[str, tuple[float, float]], pd.Series]:
...
def run(config_path: Path) -> dict[str, Any]:
    ...
    frontier_path = output_dir / "day_session_fm_side_coordinate_frontier.csv"
    frontier.to_csv(frontier_path, index=False)
    thresholds_path = output_dir / "thresholds.json"
    thresholds_path.write_text(
        json.dumps({key: {"t_up": value[0], "t_down": value[1]} for key, value in thresholds.items()}, indent=2, sort_keys=True),
        encoding="utf-8",
    )
    ...
    pd.DataFrame(
        [
            {
                "variant": "day_session_fm_side_coordinate",
    ...
    report = {
        ...
        "best_variant": "day_session_fm_side_coordinate",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run day-session-first-minute-side coordinate threshold search.")
