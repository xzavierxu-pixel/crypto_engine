...
DEFAULT_CONFIG_PATH = Path("experiments/configs/20260611_catboost_blend_diagnostics.yaml")
LEAKAGE_BLOCKLIST = {
    "target",
    "future_close",
    "abs_return",
...
def _model_params(config: dict[str, Any], model_config: dict[str, Any]) -> dict[str, Any]:
...
def _build_blends(probabilities: pd.DataFrame) -> dict[str, pd.Series]:
    rank_mean = pd.Series(
        np.mean([rankdata(probabilities[column]) / len(probabilities) for column in probabilities.columns], axis=0),
        index=probabilities.index,
    )
    return {
        "mean": probabilities.mean(axis=1),
        "median": probabilities.median(axis=1),
        "best_plus_mean": 0.5 * probabilities["d5_i400_l24_rs1"] + 0.5 * probabilities.mean(axis=1),
        "rank_mean": rank_mean,
    }
...
def run(config_path: Path) -> dict[str, Any]:
    ...
    for model_config in config["base_models"]:
        params = _model_params(config, model_config)
    ...
    results = []
    for blend_name, p_up in _build_blends(probabilities).items():
        if blend_name not in config["blends"]:
            continue
        frontier, best = _search(validation_predictions, p_up, config)
        frontier.to_csv(output_dir / f"{blend_name}_frontier.csv", index=False)
        results.append({"variant": blend_name, "validation_metrics": best})
    best_result = max(
        results,
        key=lambda row: (
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run CatBoost probability blend diagnostics.")
