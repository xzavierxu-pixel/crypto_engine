...
def get_metrics(data):
    ...
    def find_keys(d, path=""):
        ...
        if isinstance(d, dict):
            for k, v in d.items():
                if k == 'continuation_accepted_accuracy' and 'validation' in path.lower():
                    val_acc = v
                elif k == 'continuation_accepted_accuracy' and val_acc is None:
                    val_acc = v # fallback
        ...
        if f.endswith('.json'):
            ...
            try:
                with open(path, 'r', encoding='utf-8') as file:
                    content = file.read()
                    if 'continuation_accepted_accuracy' in content:
                        data = json.loads(content)
                        val_acc, acc2 = get_metrics(data)
                        
                        if val_acc is None:
                            match = re.search(r'"continuation_accepted_accuracy"\s*:\s*([\d\.]+)', content)
...
for i, item in enumerate(unique_results[:10]):
    print(f'Folder: {item[2]} | validation continuation_accepted_accuracy: {item[0]} | accepted_sample_accuracy: {item[1]}')
