...
def main() -> None:
    ...
    fixed_bid = float(config["backtest"]["fixed_bid"])
    ...
    for split, source_key, prediction_key in [
        ...
        if not np.all(all_result.bid == fixed_bid) or not np.all(all_result.bid > 0.0):
    ...
    report = {
        ...
        "primary_metric": "validation accepted-sample forced mean_accepted_pnl at absolute bid 0.50",
        "fixed_bid": fixed_bid,
    ...
    print({"validation_metrics": report["validation_metrics"], "fixed_bid": fixed_bid})
