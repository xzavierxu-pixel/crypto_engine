...
def test_train_and_live_feature_paths_match_with_book_ticker() -> None:
    ...
    settings = replace(
        ...
        derivatives=replace(
            ...
            book_ticker=replace(settings.derivatives.book_ticker, enabled=True, zscore_window=15),
        ),
    )
    spot = pd.DataFrame(
        {
    ...
    derivatives = merge_derivatives_frames(
        funding_frame=funding,
        basis_frame=basis,
        book_ticker_frame=book_ticker,
    )
