...
- Scientific protocol audit: recovered with a disclosed quarantine; see Caveat
...
The first G1–G3 runner version incorrectly fell back to the best candidate when none met the required three-positive-week tune gate and consequently read B_test once for each failed G candidate. Those three results were quarantined in `protocol_violation_btest_diagnostic.json`, removed from ranking and normal B_test metrics, and were not used to choose M, S, or hybrid parameters. The corrected artifacts mark G1–G3 as gate failures. This history means the session is artifact-complete but not strictly protocol-clean.
