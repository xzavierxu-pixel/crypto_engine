...
EXPECTED_BTEST_READS = {
    "G0_anchor_reproduction":1,"G1_high_fill_scan":0,"G2_high_fill_conservative_q":0,"G3_recalibrated_gc":0,
    "M0_market_price_qa":1,"M1_market_ev_scan":1,"M2_same_universe_comparison":0,"M3_limit_market_hybrid":1,
    "S0_fixed_stop_preflight":1,"S1_stop_grid":0,"S2_take_profit_overlay":0,"S3_combination_not_triggered":0,
}
PNL_REQUIRED = ["accepted_count","order_count","order_coverage","trade_count","fill_rate","sum_pnl","mean_accepted_pnl","mean_pnl_filled","mean_bid","win_pnl_sum","loss_pnl_sum","correct_fill_rate","realized_correct_fill_rate_submitted","wrong_submitted_count","avg_loser_cost","submitted_fill_calibration_gap","q_brier","gc_brier"]
...
def main() -> None:
    ...
    ledger_reads = {str(r.experiment_id):int(r.btest_reads) for r in ledger.itertuples()}
    btest_read_counts_exact = ledger_reads == EXPECTED_BTEST_READS
    search_names = ["G1_high_fill_scan","G2_high_fill_conservative_q","G3_recalibrated_gc","M1_market_ev_scan","M3_limit_market_hybrid","S1_stop_grid"]
    strict_holdout_search = all(not ({"w5_pnl","w6_pnl"} & set(pd.read_csv(EXPERIMENTS/name/"search.csv").columns)) for name in search_names)
    strict_holdout_winners = all(all(f in load_json(EXPERIMENTS/name/"metrics_bdev.json").get("winner_fold_metrics",{}) for f in ["w5","w6"]) for name in search_names)
    s2_dev = load_json(EXPERIMENTS/"S2_take_profit_overlay"/"metrics_bdev.json")
    s2_no_holdout_when_prerequisite_failed = all(not ({"w5","w6"} & set(c.get("folds",{}))) for c in s2_dev["candidates"])
    ...
    m_pred = pd.read_parquet(EXPERIMENTS/"M1_market_ev_scan"/"predictions_btest.parquet")
    legal_m = m_pred["has_pre_108_trade"].astype(bool)
    m_timestamp_rows_pass = bool((pd.to_datetime(m_pred.loc[legal_m,"m_trade_time"],utc=True) <= pd.to_datetime(m_pred.loc[legal_m,"market_cutoff"],utc=True)).all())
    evaluated_metrics = {
        "G0":g0,
        "S0":load_json(EXPERIMENTS/"S0_fixed_stop_preflight"/"metrics_btest.json")["variants"]["primary"],
        "M1":load_json(EXPERIMENTS/"M1_market_ev_scan"/"metrics_btest.json")["metrics"],
        "M3":m3["metrics"],
    }
    pnl_metric_coverage = {name:[field for field in PNL_REQUIRED if field not in metrics] for name,metrics in evaluated_metrics.items()}
    all_pnl_metrics_present = not any(pnl_metric_coverage.values())
    ...
    audit = {
        ...
        "btest_read_counts_exact": btest_read_counts_exact,
        "strict_two_stage_holdout_protocol": bool(strict_holdout_search and strict_holdout_winners and s2_no_holdout_when_prerequisite_failed),
        "pnl_metric_coverage_missing": pnl_metric_coverage,
        "all_required_pnl_metrics_present": all_pnl_metrics_present,
        "g0_anchor_reproduced": bool(np.isclose(g0["sum_pnl"],42.43,atol=1e-9)),
        "m0_timestamp_gate_passed": m0["qa"]["late_trade_count"] == 0 and m_timestamp_rows_pass,
        ...
        "artifact_completion_passed": bool(all_files and all_predictions and all_leakage and ledger_complete and all_pnl_metrics_present and not protected_changes),
        "scientific_protocol_completion_passed": bool(not quarantined and btest_read_counts_exact and strict_holdout_search and strict_holdout_winners and s2_no_holdout_when_prerequisite_failed and m_timestamp_rows_pass),
        "best_results": {
    ...
    print(json.dumps({k:audit[k] for k in ["all_required_files_present","all_prediction_schemas_pass","all_leakage_checks_pass","ledger_complete_and_unique","btest_read_counts_exact","strict_two_stage_holdout_protocol","all_required_pnl_metrics_present","artifact_completion_passed","scientific_protocol_completion_passed","protocol_clean","quarantined_protocol_violations"]},indent=2))
