    ...
    def transform(
        ...
        for prefix, columns in pairwise_groups:
            for left_index in range(len(columns)):
                for right_index in range(left_index + 1, len(columns)):
                    ...
                    left = df[left_name]
                    right = df[right_name]
                    feature_map[f"{prefix}_spread__{left_name}__{right_name}"] = left - right
                    feature_map[f"{prefix}_ratio__{left_name}__{right_name}"] = _safe_ratio(left, right)

        for left_name in returns:
            left = df[left_name]
            for right_name in realized_vol:
                right = df[right_name]
                feature_map[f"ret_vol_product__{left_name}__{right_name}"] = left * right
                feature_map[f"ret_vol_ratio__{left_name}__{right_name}"] = _safe_ratio(left, right)
            for right_name in relative_volume:
                right = df[right_name]
                feature_map[f"ret_relative_volume_product__{left_name}__{right_name}"] = left * right
            for right_name in volume_z:
                right = df[right_name]
                feature_map[f"ret_volume_z_product__{left_name}__{right_name}"] = left * right
            for right_name in range_pos:
                right = df[right_name]
                feature_map[f"ret_range_pos_product__{left_name}__{right_name}"] = left * right
            for right_name in close_z:
                right = df[right_name]
                feature_map[f"ret_close_z_product__{left_name}__{right_name}"] = left * right
            for right_name in efficiency:
                right = df[right_name]
                feature_map[f"ret_efficiency_product__{left_name}__{right_name}"] = left * right

        for left_name in volume_z:
            left = df[left_name]
            for right_name in relative_volume:
                right = df[right_name]
                feature_map[f"volume_state_product__{left_name}__{right_name}"] = left * right

        for left_name in range_pos:
            left = df[left_name]
            for right_name in close_z:
                right = df[right_name]
                feature_map[f"price_location_product__{left_name}__{right_name}"] = left * right
        ...
        for left_name in scalar_context:
            ...
            for right_name in returns:
                right = df[right_name]
                feature_map[f"context_ret_product__{left_name}__{right_name}"] = left * right
            for right_name in relative_volume:
                right = df[right_name]
                feature_map[f"context_relative_volume_product__{left_name}__{right_name}"] = left * right
        ...
        for column_name in stability_columns:
            ...
            for window in (6, 12, 24):
                ...
                feature_map[f"{column_name}_mean_gap_{window}"] = series - rolling_mean
        ...
        for column_name in transition_columns:
            series = df[column_name]
            feature_map[f"{column_name}_delta_1"] = series.diff(1)
            feature_map[f"{column_name}_delta_3"] = series.diff(3)
            feature_map[f"{column_name}_delta_6"] = series.diff(6)
