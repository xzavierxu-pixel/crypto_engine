...
def compare_frames(train_report: dict[str, Any], valid_report: dict[str, Any]) -> dict[str, Any]:
    ...
    train_missing = {row["column"]: float(row["null_pct"]) for row in train_report["missing_report"]}
    valid_missing = {row["column"]: float(row["null_pct"]) for row in valid_report["missing_report"]}
...
def write_reports(
    *,
    ...
    summary_lines = [f"Feature Scope: {'model features only' if model_features_only else 'full training frame'}", ""]
