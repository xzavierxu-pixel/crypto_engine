...
from src.core.constants import DEFAULT_ABS_RETURN_COLUMN, DEFAULT_TIMESTAMP_COLUMN
...
def build_abs_return_frame(
    ...
    abs_return = ((future_close - normalized["open"]) / normalized["open"]).abs()
    abs_return[future_close.isna()] = pd.NA
    return pd.DataFrame(
        {
