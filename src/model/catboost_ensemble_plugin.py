...
class CatBoostRankEnsemblePlugin(ModelPlugin):
    name = "catboost_rank_ensemble"
    ...
    def fit(
        ...
        for seed in self.seeds:
            ...
            self.models.append(model)
            self.reference_scores.append(np.sort(train_probabilities.astype("float64")))
        return self

    def predict_proba(self, X: pd.DataFrame) -> pd.Series:
        if not self.models or not self.reference_scores:
            raise ValueError("CatBoostRankEnsemblePlugin has not been fitted.")
        ranked_probabilities = []
        for model, reference in zip(self.models, self.reference_scores):
            probabilities = model.predict_proba(X)[:, 1]
            ranks = np.searchsorted(reference, probabilities, side="right") / max(len(reference), 1)
            ranked_probabilities.append(ranks)
        averaged = np.mean(ranked_probabilities, axis=0)
        return pd.Series(averaged, index=X.index, name="p_up")
    ...
    @classmethod
    def load(cls, path: str | Path) -> "CatBoostRankEnsemblePlugin":
        with Path(path).open("rb") as handle:
            payload = pickle.load(handle)
        plugin = cls(**payload["params"])
        plugin.models = payload["models"]
        plugin.reference_scores = payload["reference_scores"]
        return plugin
