...
def normalize_options_frame(
    ...
    ensure_columns(frame, [resolved_timestamp_column, "atm_iv_near"])
    ...
    if "iv_term_slope" not in normalized.columns:
        normalized["iv_term_slope"] = pd.NA
...
def load_options_frame(
    ...
    if suffix == ".csv":
        ...
        raise ValueError(f"Unsupported options input format: {resolved_path.suffix}")
    return normalize_options_frame(frame, timestamp_column=timestamp_column)
