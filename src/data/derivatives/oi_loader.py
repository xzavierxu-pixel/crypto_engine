...
def normalize_oi_frame(
    ...
    ensure_columns(frame, [resolved_timestamp_column, "open_interest"])
    ...
    if "oi_notional" not in normalized.columns:
        normalized["oi_notional"] = pd.NA
...
def load_oi_frame(
    ...
    if suffix == ".csv":
        ...
        raise ValueError(f"Unsupported OI input format: {resolved_path.suffix}")
    return normalize_oi_frame(frame, timestamp_column=timestamp_column)
